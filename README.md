...
The bot never puts an unplayed player's number into the shared message. Face-up cards are only cards already played in the center; hidden cards are represented by card-back buttons. It uses `answerCallbackQuery(show_alert=True)` for the private reveal.
